...
def test_rpcanet(folder_path, views):
    final_results_dict = {v: {w: [] for w in views} for v in views}
    for exp in tqdm(os.listdir(folder_path)):
        ...
        rpca_path, stride = config['rpca_path'], config['stride']
        val_dataset = RPCAEvaluationDataset(images_path, rpca_path, gt_path, training_view, input_shape, output_shape, stride=stride) 
        # evaluate training scene
        validation_results_dict = evaluate_rpcanet(val_dataset, model, device)
        final_results_dict[training_view][training_view].append(validation_results_dict)
        # evaluate test scenes
        for test_view in views:
            if test_view != training_view:
                test_dataset = RPCAEvaluationDataset(images_path, rpca_path, gt_path, test_view, input_shape, output_shape, stride=stride)
                test_results_dict = evaluate_rpcanet(test_dataset, model, device)
                final_results_dict[training_view][test_view].append(test_results_dict)
    # return results 
...
def test_unet(folder_path, views, n_train_list):
    ...
    for exp in tqdm(os.listdir(folder_path)):
        ...
        # initialize training/validation dataset
        images_path, gt_path, input_shape, output_shape = config['images_path'], config['gt_path'], config['input_shape'], config['output_shape']
        dataset = BFSEvaluationDataset(images_path, gt_path, training_view, input_shape, output_shape)
        ...
        for test_view in views:
            if test_view != training_view:
                test_dataset = BFSEvaluationDataset(images_path, gt_path, test_view, input_shape, output_shape)
...
def test_sequence_unet(folder_path, views, n_train_list):
    final_results_dict = {n: {v: {w: [] for w in views} for v in views} for n in n_train_list}
    for exp in tqdm(os.listdir(folder_path)):
        experiment_path = os.path.join(folder_path, exp)
        # load model
        ...
        images_path, gt_path, input_shape, output_shape = config['images_path'], config['gt_path'], config['input_shape'], config['output_shape']
        rpca_path, stride = config['rpca_path'], config['stride']
        dataset = RPCAEvaluationDataset(images_path, rpca_path, gt_path, training_view, input_shape, output_shape, stride=stride)
        training_indices = np.random.choice(np.arange(len(dataset)), size=n_train, replace=False)
        validation_indices = np.array([i for i in np.arange(len(dataset)) if i not in training_indices])
        val_dataset = Subset(dataset, validation_indices)
        # evaluate training scene
        validation_results_dict = evaluate_sequence_unet(val_dataset, model, device)
        final_results_dict[n_train][training_view][training_view].append(validation_results_dict)
        # evaluate test scenes
        for test_view in views:
            if test_view != training_view:
                test_dataset = RPCAEvaluationDataset(images_path, rpca_path, gt_path, test_view, input_shape, output_shape, stride=stride)
                test_results_dict = evaluate_sequence_unet(test_dataset, model, device)
                final_results_dict[n_train][training_view][test_view].append(test_results_dict) 
    # return results 
    return final_results_dict

def test_graynet(folder_path, views, n_train_list):
    final_results_dict = {n: {v: {w: [] for w in views} for v in views} for n in n_train_list}
    for exp in tqdm(os.listdir(folder_path)):
        experiment_path = os.path.join(folder_path, exp)
        # load model
        ...
        val_dataset = Subset(dataset, validation_indices)
        # evaluate training scene
        validation_results_dict = evaluate_graynet(val_dataset, model, device)
        final_results_dict[n_train][training_view][training_view].append(validation_results_dict)
        # evaluate test scenes
        for test_view in views:
            if test_view != training_view:
                test_dataset = RPCAEvaluationDataset(images_path, rpca_path, gt_path, test_view, input_shape, output_shape, stride=stride)
                test_results_dict = evaluate_graynet(test_dataset, model, device)
                final_results_dict[n_train][training_view][test_view].append(test_results_dict)         
    # return results 
    return final_results_dict
